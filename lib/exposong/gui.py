...
class ESTable(Gtk.Table):
    ...
    def __init__(self, rows=1, cols=1, row_spacing=6, auto_inc_y=False):
        super(Gtk.Table, self, rows, cols*2).__init__()
    ...
    def _set_options(self, kw):
        ...
        kw.setdefault('xoptions', Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL)
        kw.setdefault('yoptions', Gtk.AttachOptions.FILL)
    ...
    def attach_color(self, value, alpha=False, **kw):
        ...
        if isinstance(value[0], tuple):
            ...
            hbox = Gtk.HBox()
            for v in value:
                button = Gtk.ColorButton(Gdk.Color(*map(int, v[:3])))
                if(alpha):
            ...
            button = Gtk.ColorButton(Gdk.Color(*map(int, value[:3])))
            if(alpha):
                button.set_use_alpha(True)
    ...
    def attach_folderchooser(self, value=None, **kw):
        ...
        widget = Gtk.FileChooserButton( _("Choose File") )
        widget.set_width_chars(15)
        widget.set_action(Gtk.FileChooserAction.SELECT_FOLDER)
        if value:
    ...
    def attach_hseparator(self, **kw):
        s = Gtk.HSeparator()
        kw.setdefault('xoptions', Gtk.AttachOptions.FILL)
        self.attach_widget(s, **kw)
        return s
    
    def attach_label(self, label, x=0, y=0, w=1, h=1, **kw):
        'Add a label widget.'
        label2 = Gtk.Label(label=label)
...
def append_textview(table, label, value, top):
    ...
    table.attach(scroll, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL,
            Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, WIDGET_SPACING)
    return textview
...
def append_file(table, label, value, top):
    ...
    table.attach(filech, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return filech
...
def append_folder(table, label, value, top):
    ...
    filech.set_width_chars(15)
    filech.set_action(Gtk.FileChooserAction.SELECT_FOLDER)
    if value:
...
def append_color(table, label, value, top, alpha=False):
    ...
    if isinstance(value[0], tuple):
        ...
        cnt = 2
        for v in value:
            button = Gtk.ColorButton(Gdk.Color(int(v[0]), int(v[1]), int(v[2])))
            if(alpha):
                button.set_use_alpha(True)
            table.attach(button, cnt, cnt+1, top, top+1,
                         Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
            cnt += 1
            buttons.append(button)
        return buttons
    else:
        button = Gtk.ColorButton(Gdk.Color(int(value[0]), int(value[1]),
                                 int(value[2])))
        table.attach(button, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0,
                     WIDGET_SPACING)
        if(alpha):
            button.set_use_alpha(True)
            #button.set_alpha(int(value[3]))
...
def append_combo(table, label, options, value, top):
    ...
    for i in range(len(options)):
        ...
        if options[i] == value:
            combo.set_active(i)
    table.attach(combo, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return combo
...
def append_radio(table, label, active, top, group=None):
    ...
    table.attach(radio, 1, 2, top, top+1, Gtk.AttachOptions.FILL, 0, _LABEL_SPACING)
    return radio
...
def append_language_combo(table, value, top):
    ...
    if value:
        lang.get_child().set_text(value)
    #cell = Gtk.CellRendererText()
    #lang.pack_start(cell, True)
    #lang.add_attribute(cell, 'text', 0)
    table.attach(lang, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return lang

def append_checkbutton(table, label, buttonlabel, top):
    set_label(table, label, top)
    cb = Gtk.CheckButton(buttonlabel)
    table.attach(cb, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return cb

def append_font_button(table, label, fontname, top):
    set_label(table, label, top)
    fb = Gtk.FontButton(fontname)
    table.attach(fb, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return fb
...
def append_hscale(table, label, adjustment, top, digits=0):
    ...
    s.set_digits(digits)
    table.attach(s, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return s

def append_hbox(table, label, hbox, top):
    'Adds a HBox widget to a table and returns it.'
    set_label(table, label, top)
    table.attach(hbox, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return hbox

def append_widget(table, label, widget, top):
    'Adds any Gtk.Widget to a table and returns it'
    set_label(table, label, top)
    table.attach(widget, 2, 4, top, top+1, Gtk.AttachOptions.EXPAND|Gtk.AttachOptions.FILL, 0, WIDGET_SPACING)
    return widget
...
def append_section_title(table, title, top):
    ...
    label.set_markup("<b>"+title+"</b>")
    label.set_alignment(0.0, 1.0)
    table.attach(label, 0, 4, top, top+1, Gtk.AttachOptions.FILL, 0, 0)
    return label
...
def append_comment(table, title, top):
    ...
    if isinstance(table, Gtk.Table):
        table.attach(label, 0, 4, top, top+1, Gtk.AttachOptions.EXPAND, 0, 0)
...
def append_separator(table, top):
    sep = Gtk.HSeparator()
    table.attach(sep, 0, 4, top, top+1, Gtk.AttachOptions.FILL, 0, 0)
    return sep
...
def set_label(table, label, top):
    ...
    if isinstance(label, Gtk.Widget):
        table.attach(label, 1, 2, top, top+1, Gtk.AttachOptions.FILL, Gtk.AttachOptions.FILL, _LABEL_SPACING)
...
def filechooser_preview(file_chooser, preview):
    ...
    try:
        if filename not in __pb_cache:
            __pb_cache[filename] = GdkPixbuf.Pixbuf.new_from_file_at_size(filename,
                                                                        256, 256)
